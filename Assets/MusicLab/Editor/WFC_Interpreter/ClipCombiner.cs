...
namespace MusicForge
{
    public class ClipCombiner : MonoBehaviour
    {
        ...
        /// <param name="clips"></param>
        /// <returns></returns>
        public static AudioClip Cocatenate(AudioClip[] clips, float crossFadeDuration, bool cutLastClipCrossfade)
        {
            int samplesToFade = milisecondsToSamples(crossFadeDuration, clips[0].frequency, clips[0].channels);
            
            //Cuts the last clip to take out the silence at the end 
            if (cutLastClipCrossfade)
            {
                float[] floatSamplesLastClip = new float[clips[clips.Length-1].samples * clips[clips.Length - 1].channels];
                float[] newLastClip = new float[clips[clips.Length-1].samples * clips[clips.Length - 1].channels - samplesToFade];
                clips[clips.Length - 1].GetData(floatSamplesLastClip, 0);

                for (int i = 0; i < newLastClip.Length; i++)
                {
                    newLastClip[i] = floatSamplesLastClip[i];
                }

                clips[clips.Length - 1]= AudioClip.Create("Combine", newLastClip.Length / 2, 2, 44100, false);
                clips[clips.Length - 1].SetData(newLastClip, 0);
            }
