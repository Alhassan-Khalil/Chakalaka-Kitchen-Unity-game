...
namespace WFC
{
    public class Wave_Function_Collapse : I_GridOverride
    {
        ...
        protected Cell Collapse_Cell()
        {
            ...
            {
                UnityEngine.Debug.Log("No collapsable cells");
                return null;
            }
        }
        ...
        /// </summary>
        public void OverrideGridForMusic()
        {
            ...
            OverrideRowEntropy(0, FirtsRow.ToArray());
        ...
        public void ForceCellCollapse(int x, int y, bool debug)
        {
            m_grid[y, x].collapseCell();
            updateEntropy();
            if (debug)
                Debug_Grid();
        ...
        public void ForceRowColapse(int row, bool debug)
        {
            for (int col = 0; col < m_grid.GetLength(1); col++)
            {
                ForceCellCollapse(col, row, debug);
            }
        }
        public Cell[,] Run(bool debug)
        {
            ...
            } while (m_stop_flag == false && m_tries <= 100);

            if (m_tries >= 100)
            {
                m_grid = null;
                UnityEngine.Debug.Log("Unable to create song. Please check the rules are correct");
            }
            return m_grid;
        }
    }
